...
const updateProperty = async () => {
  ...
  try {
    await axios.put("/properties/" + product.value.id, {
      name: product.value.name,
      type: product.value.type,
      address: product.value.address,
      size: product.value.size,
...
const deleteProperty = async () => {
  try {
    await axios.delete("/properties/" + product.value.id);
...
const handleUpload = (e) => {
  ...
  axios.post("/picture", formData).then((response) => {
      ...
      <div class="col-12">
        <Toolbar class="mb-4">
          <template v-slot:start>
            <div class="my-2 flex">
              <Button label="New" icon="pi pi-plus" class="p-button-primary " id="button" @click="openNew" />
              <FileUpload mode="basic" accept="image/*" :maxFileSize="1000000" label="Upload" chooseLabel="Upload"
                class="ml-2 p-button-help" @change="handleUpload" />
            </div>
          </template>
        </Toolbar>
        ...
        <DataTable ref="dt" :value="products" dataKey="id" :paginator="true" :rows="10" :filters="filters"
          ...
          <Column header="Image" headerStyle="width:14%; min-width:10rem;">
            <template #body="slotProps">
              ...
              <img :src="'http://localhost/RealEstateApi/storage/app/public/' + slotProps.data.image_path"
          ...
          <Column field="bedrooms_number" header="Bedrooms number" :sortable="true"
          ...
          <Column field="bathrooms_number" header="Bathrooms number" :sortable="true"
            headerStyle="width:14%; min-width:10rem;">
            <template #body="slotProps">
        ...
        <Dialog v-model:visible="productDialog" :style="{ width: '450px' }" header="Property Details" :modal="true"
          class="p-fluid">
          <img :src="'http://localhost/RealEstateApi/storage/app/public/' + product.image_path"
            v-if="product.image_path" width="150" class="mt-0 mx-auto mb-5 block shadow-2" />
          <div class="field">
          ...
          <div class="field">
            ...
            <InputText id="type" v-model.trim="product.type" required="true" autofocus
          ...
          <div class="field">
            ...
            <InputNumber id="year" v-model="product.year" :class="{ 'p-invalid': submitted && !product.year }"
              :required="true" mode="decimal" :useGrouping="false" />
            <small class="p-invalid" v-if="submitted && !product.year">Year is required.</small>
          </div>
          <div class="field">
            <label for="size">Size</label>
            <InputText id="size" v-model="product.size" :class="{ 'p-invalid': submitted && !product.size }"
              :required="true" />
          ...
          <div class="formgrid grid">
            <div class="field col">
              ...
              <InputNumber id="price" v-model="product.price" prefix="$"
                :class="{ 'p-invalid': submitted && !product.price }" :required="true" mode="decimal"
                :useGrouping="false" />
        ...
        <Dialog v-model:visible="deleteProductDialog" :style="{ width: '450px' }" header="Confirm" :modal="true">
          <div class="flex align-items-center justify-content-center">
            ...
            <span v-if="product">Are you sure you want to delete <b> {{ product.type }} located in {{ product.address }}
                with an ID of {{ product.id }}</b>?</span>
          </div>
          <template #footer>
